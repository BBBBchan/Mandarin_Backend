...
@RestController
public class LibrarianRegisterHandler {

    @Autowired
    private AccountService accountService;

    @PostMapping("admin/register-librarian")
    public ResponseEntity<Map<String, Object>> registerlibrarian(@RequestBody Map params) {
        ...
        if (username.isEmpty() || email.isEmpty()) {
            ...
            if (m.matches() && email.matches(Email)) {
                ...
                if (us.size() == 0) {
                    ...
                    registers.setIdentity(2);
                    if (password.isEmpty()) {
                        registers.setPassword("00010001");
                        accountService.save(registers);
                        message = "注册成功，初始密码是00010001";
                        res.put("statues", 200);
                    } else {
